...
const PROVIDER_SPECS = [
  {
    ...
    filters_schema: [
      { key: "vote", label: "评分 ≥", kind: "float", default: 7, hint: "低于此评分的条目将被过滤，≤0 不启用" },
...
const POSTERS = Array.from({ length: 18 }, (_, i) => `https://picsum.photos/seed/asa${i + 1}/240/360`);
const TITLES = [
  ["沙丘 第二部", "2024", "电影"],
  ["奥本海默", "2023", "电影"],
  ["幕府将军", "2024", "电视剧"],
...
const PROVIDER_IDS = PROVIDER_SPECS.map((s) => s.provider_id);
Object.fromEntries(PROVIDER_SPECS.map((s) => [s.provider_id, s.provider_name]));
function pad(n) {
...
let HISTORY$1 = buildHistory(58);
const SUB_STATES = ["R", "R", "R", "S", "P", "N"];
...
let SUBSCRIBES = buildSubs(43);
function yearOk(year, ymin, ymax) {
...
function computeStats() {
  ...
  return { total: HISTORY$1.length, by_status, by_provider };
}
const ENABLED = { douban: true, maoyan: true, popular: false, mikan: true, netflix: false };
...
function _L(text, lang) {
  if (!lang || lang === "zh-CN" || !text) return text;
  return TEXT[text] && TEXT[text][lang] || text;
}
function localizeSpec(spec, lang) {
  if (lang === "zh-CN") return spec;
  const loc = (f) => ({
    ...f,
  ...
  return {
    get(url) {
      ...
      if (path.endsWith("/history")) {
        ...
        const inSet = (v, csv) => {
          const s = (csv || "").split(",").filter(Boolean);
          return !s.length || s.includes(v);
        };
        ...
        const list = HISTORY$1.filter((r) => inSet(r.provider, p.provider) && inSet(r.status, p.status) && inSet(r.type, p.mtype) && (!kw || String(r.title || "").toLowerCase().includes(kw)) && yearOk(r.year, ymin, ymax));
    ...
    post(url, body) {
      ...
      if (url.endsWith("/history/delete")) {
        HISTORY$1 = HISTORY$1.filter((r) => r.unique !== body?.unique);
        return delay({ code: 0, message: "删除成功" });
      }
      if (url.endsWith("/history/batch-delete")) {
        const ids = new Set(body?.uniques || []);
        HISTORY$1 = HISTORY$1.filter((r) => !ids.has(r.unique));
        return delay({ code: 0, message: `已删除 ${ids.size} 条`, removed: ids.size });
      }
      if (url.endsWith("/history/recognize")) {
        const rec = HISTORY$1.find((r) => r.unique === body?.unique);
...
const {createTextVNode:_createTextVNode$7,resolveComponent:_resolveComponent,withCtx:_withCtx,createVNode:_createVNode$2h,toDisplayString:_toDisplayString,unref:_unref,normalizeStyle:_normalizeStyle,createElementVNode:_createElementVNode,openBlock:_openBlock,createBlock:_createBlock,createCommentVNode:_createCommentVNode} = await importShared('vue');


const _hoisted_1 = { class: "preview-root" };
const _hoisted_2 = { class: "dash-controls mb-4" };
const _hoisted_3 = { class: "dash-grid" };
const _hoisted_4 = { class: "dash-cell" };

const {computed: computed$1_,getCurrentInstance,reactive: reactive$3,ref: ref$Q} = await importShared('vue');
...
const i18nStub = reactive$3({ locale: 'zh-CN' });
getCurrentInstance().appContext.config.globalProperties.$i18n = i18nStub;
const langLabel = computed$1_(() => LANG_LABEL[i18nStub.locale]);
function cycleLang() {
  const i = LANGS.indexOf(i18nStub.locale);
  i18nStub.locale = LANGS[(i + 1) % LANGS.length];
}

const tab = ref$Q('config');
const previewWidth = ref$Q(1120);
const dashBorder = ref$Q(true);
const snackbar = reactive$3({ show: false, text: '', color: 'success' });

const isDark = computed$1_(() => theme.global.current.value.dark);
const frameStyle = computed$1_(() => ({ maxWidth: `${previewWidth.value}px` }));
  ...
  return (_openBlock(), _createBlock(_component_v_app, null, {
    default: _withCtx(() => [
      _createVNode$2h(_component_v_app_bar, {
        ...
        append: _withCtx(() => [
          _createVNode$2h(_component_v_btn_toggle, {
          ...
          _createVNode$2h(_component_v_btn, {
            ...
            default: _withCtx(() => [
              _createTextVNode$7(_toDisplayString(langLabel.value), 1)
            ]),
            _: 1
          }),
          _createVNode$2h(_component_v_btn, {
            "prepend-icon": isDark.value ? 'mdi-weather-night' : 'mdi-weather-sunny',
            variant: "tonal",
            onClick: toggleTheme
          }, {
            default: _withCtx(() => [
              _createTextVNode$7(_toDisplayString(isDark.value ? '深色' : '浅色'), 1)
            ]),
            _: 1
          }, 8, ["prepend-icon"])
        ]),
        default: _withCtx(() => [
          _createVNode$2h(_component_v_app_bar_title, { class: "text-body-1 font-weight-bold" }, {
            default: _withCtx(() => _cache[7] || (_cache[7] = [
              _createTextVNode$7("自动订阅助手 · 组件预览")
            ])),
            _: 1
          })
        ]),
                  ...
                  default: _withCtx(() => [
                    _createElementVNode("div", {
                      ...
                      _createVNode$2h(ConfigComponent, {
                        ...
                        api: _unref(api),
                        onSave: onSave,
                        onSwitch: _cache[2] || (_cache[2] = $event => (tab.value = 'page')),
                        onClose: onClose,
                        onLayout: onLayout
                      }, null, 8, ["api"])
                    ], 4)
                ...
                _createVNode$2h(_component_v_window_item, { value: "page" }, {
                  default: _withCtx(() => [
                    _createElementVNode("div", {
                      ...
                      _createVNode$2h(PageComponent, {
                        api: _unref(api),
                        show_switch: true,
                        onSwitch: _cache[3] || (_cache[3] = $event => (tab.value = 'config')),
                        onClose: onClose
                      }, null, 8, ["api"])
                    ], 4)
                  ]),
                  ...
                  default: _withCtx(() => [
                    _createElementVNode("div", _hoisted_2, [
                      _createVNode$2h(_component_v_switch, {
                        modelValue: dashBorder.value,
                        "onUpdate:modelValue": _cache[4] || (_cache[4] = $event => ((dashBorder).value = $event)),
                        color: "primary",
                        density: "compact",
                        "hide-details": "",
                        inset: "",
                    ...
                    _createElementVNode("div", _hoisted_3, [
                      _createElementVNode("div", _hoisted_4, [
                        _cache[14] || (_cache[14] = _createElementVNode("div", { class: "dash-cell__cap" }, "仪表盘部件（宿主网格内约 1/2 宽）", -1)),
                        (!dashBorder.value)
                          ...
                          : (_openBlock(), _createBlock(DashboardComponent, {
                              key: 1,
                              config: dashConfig,
                              "allow-refresh": true,
                              api: _unref(api)
...
const App = /*#__PURE__*/_export_sfc(_sfc_main, [['__scopeId',"data-v-f52cc586"]]);
